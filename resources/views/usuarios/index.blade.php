...
<div class="container mt-4">
    <div class="card shadow-sm border-0" style="border-radius: 12px;">
                    ...
                    <tbody>
                        ...
                        <tr class="border-bottom">
                            ...
                            <td class="text-center">
                                {{-- Bloqueo: Solo el admin principal o el usuario con ID 1 --}}
                                ...
                                @else
                                    <div class="d-flex justify-content-center gap-2">
                                        {{-- Botón Editar: Lleva a la página de edición --}}
                                        <a href="{{ route('usuarios.edit', $user->usuario_id) }}" class="btn btn-sm btn-light border shadow-sm px-3">
                                            <i class="fas fa-edit text-dark"></i>
                                        </a>
                                        
                                        {{-- Botón Eliminar --}}
                                        <form action="#" method="POST" onsubmit="return confirm('¿Eliminar este usuario?')">
                                            @csrf
                                            @method('DELETE')
        ...
        <div class="card-footer bg-white border-top-0 py-3 text-end">
            <p class="small text-muted mb-0">Total: <strong>{{ count($usuarios) }}</strong> usuarios registrados.</p>
        </div>
    </div>
</div>
